...
========================
This file is the foundation for CNN model experiments.
It covers:
...
Also includes:
    App. A: Sanity check to verify data loading
    App. B: Map classes to top-level families using AudioSet ontology

Add the following to your training script to import the dataloaders and related utilities:
  from fsd50k_setup import(
    train_loader, val_loader, eval_loader,
    NUM_CLASSES, decode_labels, encode_labels, FRAME_COUNT,
    vocab, top_level, get_top_level_family,
    train_df, focalloss, prior_bias, mixup)
...
# Configurable options for experimentation
subset = False
randomcrop = True
oversample = True
specaug = True
...
PREPROCESSED = Path("data/preprocessed128")  # change to preprocessed data path

# Normalization constants. Must match the correct preprocessed data 
NORM_MEAN = -21.5820
NORM_STD  =  20.2802
...
# -- 2. LABEL ENCODER -- 
# maps between label names and indices
...
# -- App. B: MAP CLASSES TO FAMILIES

# This section maps the 200 classes to their top-level families (e.g. "Music", "Animal", etc.) using the AudioSet ontology. This allows for analysis at the family level and can be useful for understanding model performance across different types of sounds.
